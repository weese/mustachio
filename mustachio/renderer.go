...
func lookupInContext(ctx any, segments []string) (any, bool) {
	...
	for _, s := range segments {
		...
		if arr, ok := current.([]any); ok {
			...
			idx := -1
			// quick parse without strconv to avoid import
			if len(s) > 0 {
				idx = 0
				for _, ch := range s {
					if ch < '0' || ch > '9' {
...
func (v *varNode) render(w io.Writer, p ValueProvider, _ PartialLoader, _ delimiters) error {
	...
	if str, called, err := tryCallZeroArgLambda(val); called {
		...
		_, err = io.WriteString(w, html.EscapeString(out))
		return err
	}
	s := toString(val)
	if v.unescaped {
...
func (pn *partialNode) render(w io.Writer, p ValueProvider, partials PartialLoader, delims delimiters) error {
	...
	if pn.indent != "" {
		...
		b.Grow(len(out) + len(pn.indent)*strings.Count(out, "\n") + len(pn.indent))
		b.WriteString(pn.indent)
		for i := 0; i < len(out); i++ {
			ch := out[i]
			b.WriteByte(ch)
			if ch == '\n' && i < len(out)-1 {
		...
		out = b.String()
		// ensure trailing newline when partial was standalone and didn't end with one
...
func parseTokens(template string, tokens []token) (*rootNode, error) {
	...
	truncateIndent := func(indentStart int) {
		...
		if tn, ok := (*list)[len(*list)-1].(*textNode); ok {
			text := tn.text
			// indentStart is absolute; compute how much of tn.text to keep
			// tn.text corresponds to most recent text; we cannot easily map absolute. As an approximation,
			// drop any trailing spaces/tabs after last newline in tn.text
			if idx := strings.LastIndexByte(text, '\n'); idx >= 0 {
				// keep up to idx+1
				tn.text = text[:idx+1]
			} else {
				// entire text was indentation; drop it
				tn.text = ""
			}
		}
	}
	// skip region management for trimming trailing whitespace after standalone
	skipUntil := -1
	for i := 0; i < len(tokens); i++ {
		t := tokens[i]
		if t.typ == tText {
			// possibly skip due to earlier standalone trim
			if skipUntil >= 0 {
				if t.end <= skipUntil {
					continue
				}
